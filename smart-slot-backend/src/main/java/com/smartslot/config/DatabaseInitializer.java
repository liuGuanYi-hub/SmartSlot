...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    private void initMatchChatTable() {
        try {
            String sql = """
                CREATE TABLE IF NOT EXISTS `match_chat_message` (
            ...
            """;
            jdbcTemplate.execute(sql);
            log.info("数据库初始化: match_chat_message 搭子微室聊天表检查就绪");
        } catch (Exception e) {
            log.warn("检查或创建 match_chat_message 遇到异常: {}", e.getMessage());
        }
    }

    private void initSeedMatchChatMessages() {
        try {
            Integer count = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM `match_chat_message`", Integer.class);
            if (count != null && count > 0) return;
            ...
            """;
            jdbcTemplate.execute(insertSql);
            log.info("数据库初始化: 成功导入拼场搭子微室聊天演示消息");
        } catch (Exception e) {
            log.warn("导入初始拼场聊天消息异常: {}", e.getMessage());
        }
    }
}
